...
// 2. Array avec prénom et nom
let names = entrepreneurs.map(e => `${e.first} ${e.last}`);
console.log("Prénoms et noms des entrepreneurs :", names);

// 3. Âge actuel des entrepreneurs
let currentYear = new Date().getFullYear();
let ages = entrepreneurs.map(e => ({...e, age: currentYear - e.year}));
console.log("Âge actuel de chaque entrepreneur :", ages);

// 4. Trier les entrepreneurs par nom de famille
let sorted = entrepreneurs.sort((a, b) => a.last.localeCompare(b.last));
console.log("Entrepreneurs triés par nom de famille :", sorted);
